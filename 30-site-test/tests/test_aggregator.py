...
def test_cost_per_success():
    rows = [
        Attempt(site_id="a", arm="metro-host", geo="DE", outcome="content_ok", latency_ms=1, bytes=1, cost_usd=0.01, ts="t"),
        Attempt(site_id="b", arm="metro-host", geo="DE", outcome="reachable",  latency_ms=1, bytes=1, cost_usd=0.01, ts="t"),
    ]
    sc = aggregate(rows)
    isp = sc["arms"]["metro-host"]
